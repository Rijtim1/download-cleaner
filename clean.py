...
class Clean:
    ...
    def get_file_names(self):
        ...
        self.root_dir, self.dirs, self.file_names = os.walk(DOWNLOAD_PATH).__next__()
        if self.debug:
    ...
    def move_to_dirs(self):
        ...
        for i in range(len(self.file_names)):
            ...
            if extension in self.keys:
                shutil.move(os.path.join(self.root_dir, self.file_names[i]), os.path.join(self.root_dir, extension))
                count += 1
